## Remove Linux and Mac search and updated to only run on Windows
## Not orginal author just removed a few lines and added static path to rp++ binary
...
def add_missing_gadgets(ropper_addresses: set, in_file, outfile, bad_bytes, base_address=None):
    """for whatever reason rp++ finds significantly more gadgets, this function adds them to ropper's dump of all gadgets"""
    rp_filename = 'rp-win-x86.exe'
    rp_path = Path(rp_filename)

    if not rp_path.exists():
        print(f"[bright_red][!][/bright_red] {rp_filename} not found in the current directory.")
        return

    with tempfile.TemporaryFile(mode='w+', suffix='osed-rop') as tmp_file, open(outfile, 'a') as af:
        command = f'{rp_path} -r5 -f {in_file} --unique'
